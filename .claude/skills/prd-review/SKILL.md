...
name: prd-review
description: >
  PRD 审查与需求基线 Skill。审查业务完整性和闭环性，并输出初次审核记录；存在阻断时严格停止，
  本期阻断项获得有效证据并全部关闭后生成正式需求基线；零阻断时在审核记录后生成基线。
...
然后输出审核记录和需要与产品确认的问题。最后输出完善的 PRD。
```
...
只在 `initial-review` 执行。

按 `references/output_structure_guide.md` 的固定五问，只依据 P1 已读取的 PRD 形成轻量需求初识。该内容用于帮助设计师建立共同业务认知。

P1.5 不是事实审查、知识消费或体验定案。不得生成问题、建议、体验结论或需求事实。不得单独展示、单独写盘或等待用户回复。

完成后必须进入 P2。P8 生成初次审核记录时，将该内容作为同一文件的第一部分写入。
...
完成以上既有回查和三类判断后，按 `references/review_rules.md` 的分流规则，将场景内关注点标为业务事实补充、待确认或体验定案事项。分流只组织审核记录，不改变既有三类问题的语义。

分流前必须继承 P4 的完整场景闭环骨架。无法判定时：

- 影响业务闭环：返回 P6 的候选问题回查，保留为待确认。
- 不影响业务闭环：不输出，不得标为体验定案事项。
...
读取 `references/output_structure_guide.md` 第 1 至 7 节，生成初次审核记录。该记录必须包含 P1.5 的需求初识和场景审核结果。
...
每次 `initial-review` 完成 P8 后，按 `references/output_structure_guide.md` 生成：

- `spark-output/prd_review_questions.md`

存在阻断时，审核记录写入后立即停止，不生成需求基线，不推荐下游。

零阻断时，审核记录写入后直接进入需求基线门禁。

## P9 · 产品回复复核

`response-finalization` 进入本节点后，读取并执行 `references/review_rules.md` 第 17 节。回复关闭、问题移出和新候选问题的语义只以该节为准。

使用 `references/output_structure_guide.md` 第 6 节更新原审核记录中的产品原话、列表项状态、关闭依据和问题组状态。产品回复改变所在场景业务事实时，重新回查并更新该场景的业务事实补充、体验定案事项或建议修改。输出文件只记录结论，不决定问题能否关闭。
...
- 不输出体验策略。
- 只允许在审核记录中标记“体验定案事项”；不得写入体验方向、候选方案或具体交互做法。
- 不输出页面、组件、布局或具体文案。
- 不自动触发。
- 不让脚本执行语义判断。
- 不允许下游读取问题单恢复需求事实。
