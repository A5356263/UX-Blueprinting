...
完成 Step 7 的未冻结旅程草稿后、进入 Step 8 写盘前，才允许检查 `spark-output/context/page-generation-handoff.md`。该文件不存在、项目名与当前正式上游不一致或 `journey-analysis` 索引为“无”时，立即跳过，现有流程不变。

读取门禁：

1. 第一次只定位并读取 `## 0. 消费索引` 中的 `journey-analysis` 行，不读取事实正文。
2. 命中后只按该行列出的 `HF` ID 定向读取对应事实项；禁止全量读取文件或读取其他消费者的候选事实。
3. 只有事实会改变用户步骤、状态流转、权限分支、失败恢复或风险动作，且当前草稿没有明确落点时，才允许补充。

增量边界：

- 只允许追加到已有阶段的 `actions[]`、`touchpoints[]`、明确分支、恢复路径或来源说明。
- 不得改变运行模式、readiness、补问结果、旅程主方向、阶段顺序和既有阶段划分。
- 不得把 Tab、字段、控件、布局或固定文案写入旅程。
- 事实要求重构阶段或主路径时，只写入缺口说明，不自动重做旅程。
- 新增内容标为 `原文提取` 并保留对应 `HF` ID；只出现名称而没有动作、流转或结果，不算覆盖。
- 命中索引时，在 Markdown 头部数据来源之后追加一行：`页面事实增强：候选 {N}；已覆盖 {N}；新增 {N}；冲突 {N}`；未命中时不增加该行。
