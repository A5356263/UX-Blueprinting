...
description: 设计策略生成 Skill。读取 PRD 或需求材料，执行两阶段流程——Phase 1 从材料中识别用户问题并形成设计判断（含 UX 最优解、候选方向、取舍缺口），Phase 2 基于判断展开带来源分级的策略报告。当用户讨论需求分析、PRD 评审、设计方向判断，或希望从需求材料生成结构化设计策略报告时触发。
---
