...
const ROOT_KEYS = [
  ...
  "source_mode", "decision_summary", "failure_summary", "reframed_problem",
  "skipped_premises", "recommended_direction", "next_step", "out_of_scope",
  "open_questions",
];
const SOURCE_MODES = new Set(["direct-input", "uxb-inflight", "unknown"]);
...
function validate(data) {
  ...
  if (data.skill !== "product-analysis") errors.push("skill 必须为 product-analysis");
  if (data.version !== "2.0") errors.push("version 必须为 2.0");
  if (data.artifact_md !== "spark-output/product_analysis.md") {
    errors.push("artifact_md 必须为 spark-output/product_analysis.md");
  }
  for (const field of [
    "generated_at", "project_name", "source_mode", "decision_summary",
    "failure_summary", "reframed_problem", "recommended_direction", "next_step",
  ]) {
    nonEmptyString(data[field], field, errors);
  }
  if (typeof data.source_mode === "string" && !SOURCE_MODES.has(data.source_mode)) {
    errors.push("source_mode 值不合法");
  }
...
function main() {
  ...
  console.log("product-analysis context valid");
}
