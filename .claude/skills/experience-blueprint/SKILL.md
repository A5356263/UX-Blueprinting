...
如果存在加载态、空状态、审批中、已关闭、异常态，必须补“状态反馈区”。
如果存在按钮置灰、入口隐藏、阻断弹窗、字段校验，必须补“异常状态下的结构变化”。
如果存在关键提示、表单字段、弹窗或 toast，必须补“具体文案”。
...
如果抽屉包含表单，必须补充字段校验、提交前校验和提交失败反馈。
