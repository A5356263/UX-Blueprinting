...
本次调整保持 `§0-§9` 正文分析骨架，只更新上游交接和蓝图内部完整性推导。
...
在 `baseline-mode` 和 `uxb-mode` 中，本节只记录不改变当前交互方案的外部依赖。

以下信息缺失时必须停止生成正式蓝图，不得写入 `§8` 后继续：
