...
体验蓝图阶段必须独立完成本阶段的知识补充判断。允许继承第一阶梯已经定案的业务事实，不得机械重读；本阶段新增展开的体验策略不得仅依赖上游摘要或会话记忆。UXB 的知识消费服务于需求定案；体验蓝图的知识消费服务于交互设计展开。

1. 从第一阶梯正式来源提取角色、业务对象、动作、状态、风险与预期结果，明确当前体验判断问题并据此确定主域；不得按需求标题、页面名或单个关键词定域。
2. 读取 knowledge-wiki 的 `knowledge/wiki/index.md`；业务知识先按领域 README 学习实际文件路由，index 明确直达的单一设计知识直接进入，再按目标文件顶部导航只读命中章节。
3. 业务知识以承载对象与结果事实的主域为准。仅当依赖会改变交互路径、页面结构、状态反馈、异常恢复或责任边界时补读依赖域，随后回到主域确认状态、结果、异常与审计。
4. 只确认依赖能力存在时，领域 README 足够；开始具体设计依赖域的角色选择、操作节点、时序、状态、异常或恢复机制时，必须在起草对应内容前补读该依赖域的命中章节。
5. 结合第一阶梯正式来源独立判断相关性，不得只抄 UXB 已选结论；命中局部知识后不得停止，问题涉及的主体、动作、条件、例外、结果及已触发依赖均有正式依据后再输出。
6. 将知识转成体验策略，写入 `§9` 并落到正文真实章节或节点；来源与落点必须能双向反查。
...
- `index` 与 README 只负责路由；raw 是正式事实。业务入口必须按实际 README 选文件，只有 index 明确指向的单一设计知识才直接进入 raw；路由或 raw 不可用时只记知识缺口。
- 不得猜测目录、遍历 raw、逐文件试读或按关键词并列命中领域。文件内容较大且需求只命中局部主题时，必须按顶部导航读取完整命中章节；只有需求覆盖文件大部分主题时才允许全文读取。`页面与字段.md` 默认只读命中页面章节。
