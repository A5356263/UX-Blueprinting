...
- 先读 `knowledge-wiki` 的知识索引、overview 或 index。
- 从索引中定位最相关的 `wiki/summaries/...` 路由。
- 先读命中的 `summary`，再根据 `summary` 中的 `source_path` 或 raw 指向读取对应 `raw`。
- `summary` 只作为路由层，不得作为正式判断依据。
- `raw` 才能进入知识支撑依据。
- raw 读取失败时，该知识不得进入“已消费知识”，只能进入知识缺口。
- 默认读取与判断问题直接相关的 2-5 个 raw；如果主业务对象、边界、角色责任或关键状态仍无法闭合，最多可读 7 个 raw。
- 超过 5 个 raw 时，必须在输出中说明补读原因。
...
- 用 `raw/.../README.md` 替代 summary。
- 因为没搜到 `summary*.md` 就判断没有 summary。
