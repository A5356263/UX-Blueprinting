...
- 从 index 进入主域的实际入口；业务入口进入领域 README，index 明确直达的单一设计知识直接进入。
- 学习 README 的实际文件路由；进入目标文件后按顶部导航只读命中章节。条件依赖被触发时按其实际入口补读，读完回到主域闭环。
- `index` 与 README 只作为路由层，不得作为正式判断依据；index 明确直达的单一设计知识直接作为事实层。
- `raw` 才能进入知识支撑依据。
...
- 把 README 当作正式事实，或在路由未命中 raw 时补造知识。
