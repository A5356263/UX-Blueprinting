# HTML 预览执行细则

这份文件只说明体验蓝图阶段如何在 Skill 内稳定生成 HTML 预览，不扩展到预览之外的新职责。

## 1. 原则

- HTML 预览属于 `experience-blueprint` Skill 的输出职责
- 固定复用 `preview_template.html` 的骨架，不让模型临时设计结构
- 业务蓝图和体验蓝图必须同时承接到同一个 HTML
- 左侧只切换 `业务蓝图 / 体验蓝图`，不做章节级第二层 tab
- 当前蓝图正文保持整份可见，章节继续通过锚点导航访问
- 缺少组件化映射时，优先完整保留章节内容，不允许漏章节

## 2. 固定输入

- `spark-output/uxb_output.md`
- `spark-output/experience_blueprint.md`
- `references/preview_template.html`

## 3. 固定输出

- `spark-output/preview/experience_blueprint_preview.html`

## 4. 执行顺序

1. 读取 `uxb_output.md`，提取业务蓝图章节
2. 读取 `experience_blueprint.md`，提取体验蓝图章节
3. 生成左侧 `业务蓝图 / 体验蓝图` 顶层切换按钮
4. 为当前激活蓝图生成对应章节导航项
5. 将全部正式章节按固定容器映射进模板，右侧保持当前蓝图整份正文
6. 保留表格、ASCII 框图、附录和待确认问题
7. 在 Skill 内完成内容注入并输出最终 HTML

## 4.1 业务蓝图固定章节顺序

业务蓝图章节至少按以下顺序承接：

1. `§0 本次关键判断`
2. `§1 需求输入摘要`
3. `§2 业务场景判断`
4. `§3 需求成立性判断`
5. `§4 推荐业务边界`
6. `§5 角色、功能、规则范围`
7. `§6 状态、异常与边界`
8. `§7 体验蓝图承接要求`
9. `§8 约束与不做什么`
10. `§9 待确认问题`
11. `附录`

## 4.2 导航闭环要求

- `业务蓝图` 和 `体验蓝图` 的每个正式章节都必须有对应导航项
- 顶层 tab 只负责切换 `业务蓝图 / 体验蓝图`
- 章节项仍然只是锚点导航，不承担内容面板切换
- `§1` 旅程图如果使用折叠，只能折叠阶段详情块，不能折叠主路径概览、关键转折或其他章节
- `§1` 旅程图中出现几个角色路径，HTML 就必须渲染几个角色路径；不允许只完整渲染第一个角色
- `§0` 不能只有内容没有导航
- `附录` 不能只有内容没有导航
- 导航项、章节容器、锚点 id 必须一一对应
- 如果某章节降级为通用正文渲染，也不能从导航中消失

## 5. 不允许的做法

- 不允许重新设计布局
- 不允许把业务蓝图压成摘要再塞进页面
- 不允许跳过 `§0`、附录、状态表、ASCII 框图
- 不允许出现“正文有章节但导航缺失”的半对齐状态
- 不允许把生成能力外溢成当前分支的新 CLI / package 主链路
