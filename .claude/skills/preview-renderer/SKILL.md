...
执行完成后必须按以下格式分块输出，不得把主链说明混在生成结果里：
...
主链说明：
HTML 预览只是查看产物的附加动作，不会改变主链状态。
请回到上一步继续执行。
```

硬规则：

- 不读取聊天记录来推断上一个业务 skill 的交接语。
- 不读取 `shared-workflow/next-skill.md` 或 `shared-workflow/skill-graph.json` 重新生成主链推荐。
- 不在 `preview-renderer` 内维护各业务 skill 的交接语副本。
- 不输出“你可以选择：...”这类主链推荐句式。
- `主链说明` 必须作为最后一个输出块，前面必须有分隔线，不得与生成结果、状态表或文件列表混写。
