...
> 生成时请基于本次确认的需求材料中的实际角色、页面和交互路径填写，不得编造。
...
- **输入优先级**：第 <1/2/3> 层
- **主要输入源**：<体验蓝图 / 背景材料 / 需求材料>
- **补全策略**：<已有体验蓝图复用 / 背景描述场景提取 / 纯需求推导 + 显式缺口>
- **降级提示**：<!-- 如果降级到第 2/3 层，输出以下提示 -->

> ⚠️ 未基于体验蓝图生成。当前旅程基于需求文档和背景描述推导，部分交互节点为推断补全。建议先补齐体验蓝图或等效的高置信交互材料，以获得更精确的交互流程定义。
...
|---|---|---|---|
| <冲突描述> | <规则内容（引用 knowledge-wiki 中命中的相关知识）> | <需求中的主张> | Nx, Ny |

> ⚠️ 以上冲突已按需求文档生成埋点，标记为 [conflict]。建议回到业务判断阶段或等效治理流程进行正式裁定。
