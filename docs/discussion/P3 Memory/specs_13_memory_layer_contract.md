...
定义当前项目中 `Memory Layer`（记忆层）应如何对跨任务可复用的质量经验进行提炼、分类、记录、接受与消费。

本合同的目标不是建立聊天式长期记忆，也不是替代 `knowledge/wiki/` 的独立知识沉淀体系，而是在现有执行中枢、Repair Loop、Capability Registry 之上，正式建立一层 **质量经验沉淀层**。
...
#### `capability_insight`
记录与 capability 使用相关的经验，例如：
- 哪个 capability 常触发 warning
- 哪个 capability 在某任务类型下更关键
...
- `wiki_ref=knowledge/wiki/topics/permission-domain-index.md`
...
#### `source_issue_ids`
如来源于 repair / check / gate，应记录相关 issue。
...
### 作用
该文件是用户视角的查看入口，不是机器真源。

## 与 P2 / P4 的关系

### 与 P2 的关系
Memory Layer 可以消费：
- repair loop 暴露的 issue
- remediation plan
- retry scope
- resolved / accepted / deferred 状态

但 Memory Layer 不替代 Repair Loop。

### 与 P4 的关系
Memory Layer 可以围绕 capability 进行经验沉淀，例如：
- 某 capability 在什么任务类型下高风险
- 某 capability 常见 warning 模式

但 Memory Layer 不替代 Capability Registry。
