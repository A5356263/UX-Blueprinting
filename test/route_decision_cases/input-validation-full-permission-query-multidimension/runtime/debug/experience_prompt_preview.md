...
> 权威输入：`projects/input-validation-full-permission-query-multidimension/workspace/facts.md`、`projects/input-validation-full-permission-query-multidimension/workspace/business_blueprint.md`、`E:/AI设计/体验蓝图构建思路/specs/10_experience_blueprint_contract.md`、`E:/AI设计/体验蓝图构建思路/templates/experience_blueprint.template.md`
...
- 重点维度：业务能力、业务规则、权限治理、流程承接、信息结构。
- route 输出建议：重点展开能力变化、方案形态和与既有能力的关系；重点展开规则边界、校验条件、限制原因和体验承接要求；重点展开可操作条件、不可操作原因、风险提示和异常后的下一步
- 升级信号：当前已建议 full，不建议自动降级。；如果后续事实证明范围更小，也应由人工确认后再调整，不自动改为 fast 或 standard。
- route 判断理由：初判为权限与治理风险，主要压力在业务能力、业务规则、权限治理，涉及核心能力、治理或模型边界，需走完整链路。
- 写作规则以对应 specs 中的路线说明要求为准。

## 3. facts 摘要

- 输入来源：source/requirement.md 与 source/background.md，需求族为 权限查询。
- 任务目标：设计用户、角色、权限、OA 表单和报表的多维权限查询能力。
- 使用场景：管理员需要从不同业务对象反查权限来源、可见范围、审批链路和导出依据。
...
### 7. 主要风险与保护策略
- 风险：多维查询可能暴露敏感权限结构。 保护：通过校验、阻断、提示和审计记录降低影响。
