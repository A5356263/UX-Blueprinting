...
- 需求类型：局部体验优化；建议路线：fast；置信度：medium。
- 重点维度：信息结构、评审风险。
- route 输出建议：重点展开模块定义、信息分组、入口归属和页面层级；重点展开容易误解的边界、争议点和必须确认项
- 升级信号：后续发现权限、审批、数据范围、导出或状态机变化时，需升级 standard。；后续发现新增业务规则、校验条件或阻断异常决定方案成败时，需升级 standard。
- route 判断理由：初判为局部体验优化，主要压力在信息结构、评审风险，未发现会改变核心业务规则的信号。
