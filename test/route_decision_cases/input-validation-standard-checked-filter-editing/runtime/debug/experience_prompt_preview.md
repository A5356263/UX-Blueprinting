...
> 权威输入：`projects/input-validation-standard-checked-filter-editing/workspace/facts.md`、`projects/input-validation-standard-checked-filter-editing/workspace/business_blueprint.md`、`E:/AI设计/体验蓝图构建思路/specs/10_experience_blueprint_contract.md`、`E:/AI设计/体验蓝图构建思路/templates/experience_blueprint.template.md`
...
- 需求类型：业务流程调整；建议路线：standard；置信度：high。
- 重点维度：业务规则、流程承接、状态异常、评审风险。
- route 输出建议：重点展开规则边界、校验条件、限制原因和体验承接要求；重点展开主路径变化、步骤衔接、角色协同和结果回写；重点展开状态含义、异常触发、反馈形式、失败原因和补救路径
- 升级信号：后续发现新增核心业务能力、重构既有模块或改变业务对象关系时，需升级 full。；后续发现多角色协作闭环、权限模型、审批模式或状态机变化时，需升级 full。
- route 判断理由：初判为业务流程调整，主要压力在业务规则、流程承接、状态异常，需要保留业务边界判断后再转体验方案。
- 写作规则以对应 specs 中的路线说明要求为准。

## 3. facts 摘要

- 输入来源：source/requirement.md 与 source/background.md，需求族为 快捷查看功能权限。
- 任务目标：在已勾选筛选开启时保护未保存编辑状态。
- 使用场景：管理员勾选或取消权限后切换筛选，需要明确未保存变更是否仍保留。
- 筛选切换不应丢失未保存勾选。
- 半选状态需要保持一致。
...
### 7. 主要风险与保护策略
- 风险：筛选隐藏未保存变更时需要给出可见反馈。 保护：通过校验、阻断、提示和审计记录降低影响。
