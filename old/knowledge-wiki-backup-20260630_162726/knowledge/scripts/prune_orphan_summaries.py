...
def main() -> int:
    ...
    if args.apply and not args.confirm_delete:
        raise SystemExit("Deleting orphan summaries requires --confirm-delete")
    ...
    for orphan in orphans:
        rel_summary = orphan.summary_file.relative_to(root).as_posix()
        print(f"orphan=knowledge/{rel_summary}")
        print(f"missing_source_path={orphan.source_path}")
    for item in removed:
