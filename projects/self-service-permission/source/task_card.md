...
3. 生成 facts.md（详见 `specs/08_fact_extraction_contract.md`）→ 运行 `python -m packages gate-facts self-service-permission`，通过才能进入下一步
4. 生成 business_blueprint.md（详见 `specs/09_business_blueprint_contract.md`）→ 运行 `python -m packages gate-business self-service-permission`，通过才能进入下一步
...
- 执行中结果: projects/self-service-permission/workspace/
- 归档结果: projects/self-service-permission/exports/final/
