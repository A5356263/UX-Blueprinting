...
- projects/sub-admin-permission-copy/source/requirement.md
- projects/sub-admin-permission-copy/source/background.md

## Required Outputs

- projects/sub-admin-permission-copy/workspace/facts.md
- projects/sub-admin-permission-copy/workspace/business_blueprint.md
- projects/sub-admin-permission-copy/workspace/experience_blueprint.md
- projects/sub-admin-permission-copy/workspace/gap_list.md
- projects/sub-admin-permission-copy/workspace/check_report.md
- projects/sub-admin-permission-copy/workspace/check_status.json

## Read Order

1. 先读本文件
2. 再读 `Required Inputs`
3. 生成 facts.md（详见 `specs/08_fact_extraction_contract.md`）→ 运行 `python -m packages gate-facts sub-admin-permission-copy`，通过才能进入下一步
4. 生成 business_blueprint.md（详见 `specs/09_business_blueprint_contract.md`）→ 运行 `python -m packages gate-business sub-admin-permission-copy`，通过才能进入下一步
5. 生成 experience_blueprint.md（详见 `specs/10_experience_blueprint_contract.md`）→ 运行 `python -m packages gate-experience sub-admin-permission-copy`，通过才能进入下一步
6. 运行 `python -m packages validate sub-admin-permission-copy` 和 `python -m packages coverage sub-admin-permission-copy`
7. 运行 `python -m packages archive sub-admin-permission-copy`（产物镜像到 exports/）
8. 运行 `python -m packages preview sub-admin-permission-copy --no-serve`（生成静态预览 HTML 到 runtime/preview/）
...
- 执行中结果: projects/sub-admin-permission-copy/workspace/
- 归档结果: projects/sub-admin-permission-copy/exports/final/
