...
- `## 1. 旅程图` 与 `## 2. 交互流程总览` 使用统一的横向流程视觉。
- 流程节点默认不自动折行。
- 内容超宽时通过横向滚动承载，而不是切换成其他展示形式。
- `## 2. 交互流程总览` 中的 `异常流程重点` 保留在正文里，不进入流程可视化区域。
- `## 3. 主交互流程`、`## 4. 次交互流程`、`## 5. 异常与阻断流程`、`## 6. 页面 / 弹窗 / 抽屉设计`、`## 7. 状态与反馈文案`、`## 8. 待确认问题` 保持文档式排版，不统一卡片化。
