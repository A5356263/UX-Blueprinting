...
进入 `Step 2` 并确定本次重点体验维度后，对应设计知识必须在输出 `Step 2` 前就已经读过，并进入判断单要消费的资料集合；不要把这件事留给工程主链路后补。
...
1. `files` 列分析阶段已消费、且后续正式产物仍需参考的知识文件。分析阶段读过的 raw，只要与本次任务的核心对象、流程、规则、体验相关，都应进入 `files`。不要为了“精简”排除已读且相关的文件。
2. `reasoning` 只说明为什么选这些知识，不写业务方案
3. 不为了“更完整”把整域 summary 或 raw 全量塞进去；也不要为了“最小”把分析阶段实际消费过的相关文件排除。
4. raw 可以直接进入 `files`，前提是你能说明它为什么必要
5. 一旦某类知识被命中，执行顺序仍然保持：先读 `summary`，再读对应 `raw`，再进入分析和判断单写入

在填写 `knowledge_selection.reasoning` 前，默认先完成这条内部收束链：

```text
先明确当前待判断问题
→ 判断这些问题需要哪些对象 / 角色 / 规则 / 状态 / 跨领域动作证据
→ 记录实际读取并使用的知识
→ 判断当前结论属于已确认事实 / 知识支持的推断 / 设计判断 / 必须确认的缺口
→ 再压缩写入 reasoning
```

这条内部收束链是 authoring 约束，不是新增判断单字段，也不是要求把完整推理过程原样写进 JSON。

## 字段写法

- `business_depth`：写复杂度驱动后的业务输出深度，不写分析过程
- `knowledge_selection.files`：列分析阶段已消费、且后续正式产物仍需参考的知识文件
- `knowledge_selection.reasoning`：只说明选这些知识的原因，不写业务方案；至少说明这些知识解决了什么待判断问题、支撑了哪些事实 / 推断 / 设计判断、还剩哪些关键缺口
...
`judgment.reason` 可以顺手交代阻断型不确定项是否仍存在；但不要把它写成 `knowledge_selection.reasoning` 的替代品，也不要把“资料相关性”这类空话重复塞进两个字段。
