# 00_domain_overview

- page_id: PG-SUMMARY-BUSINESS-BUSINESS-HR_SERVICE-RECRUITMENT-00_DOMAIN_OVERVIEW
- page_type: summary
- source_path: knowledge/raw/business/hr_service/recruitment/00_domain_overview.md
- source_group: business
- status: active
- confidence: medium
- summary_role: ai_route_card
- semantic_status: ai_generated
- semantic_updated_at: 2026-05-08
- updated_at: 2026-05-08
- source_refs: [knowledge/raw/business/hr_service/recruitment/00_domain_overview.md]
- related_summaries:
  - knowledge/wiki/summaries/business/hr_service/recruitment/10_recruitment_management.md
  - knowledge/wiki/summaries/business/hr_service/recruitment/README.md
  - knowledge/wiki/summaries/business/account_and_enterprise_lifecycle/00_domain_overview.md
  - knowledge/wiki/summaries/business/app_management/00_domain_overview.md
  - knowledge/wiki/summaries/business/approval_management/00_domain_overview.md

## 1. 知识定位

招聘管理子域总览，说明Moka集成覆盖范围和与其他域的关系。

## 2. 任务触发线索

涉及招聘管理边界、Moka开通条件、与入职衔接关系时读取。

## 3. 覆盖内容

- Moka开通条件（超管+高级认证）
- 组织架构同步、账号角色管理
- 招聘→入职的Offer同步链路

## 4. 可直接使用的稳定结论

- 薪福通与Moka免密登录
- Moka三种角色：超管、HR、用人经理
- 同步字段：姓名、手机号、部门、工作地、入职日期、证件信息、教育经历、工作经历

## 7. 邻近阅读
- knowledge/wiki/summaries/business/hr_service/recruitment/README.md
- knowledge/wiki/summaries/business/hr_service/recruitment/10_recruitment_management.md
- knowledge/wiki/summaries/business/hr_service/employee_management/16_employment_lifecycle.md
> summary_path: knowledge/wiki/summaries/business/hr_service/recruitment/00_domain_overview.md

## 5. 必须回查 raw 的情况

以下情况不能只读 summary：

- 需要完整规则细节或精确条款时
- 需要正式证据或原文引用时
- 需要页面或流程的完整描述时
- 涉及 [GAP] / [CONFLICT] / [QUESTION] 标记项时
- summary 无法覆盖当前判断点或信息量不足时

## 6. 缺口 / 冲突 / 不确定项

- none

## 7. 邻近阅读

弱指向 3-5 个相关 summary。

- knowledge/wiki/summaries/business/hr_service/recruitment/10_recruitment_management.md
- knowledge/wiki/summaries/business/hr_service/recruitment/README.md
- knowledge/wiki/summaries/business/account_and_enterprise_lifecycle/00_domain_overview.md
- knowledge/wiki/summaries/business/app_management/00_domain_overview.md
- knowledge/wiki/summaries/business/approval_management/00_domain_overview.md

> summary_path: knowledge/wiki/summaries/business/hr_service/recruitment/00_domain_overview.md
